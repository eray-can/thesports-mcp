...
**Endpoint**: `GET /v1/football/player/honor/list`

**Plan / Category**: `DATABASE DATA`

**Included in Your Plan**: `No ❌`

**Description**: Return full player honor data，and obtain new or changed data according to the time<br/><br/>1、Full update for the first time，full data is obtained according to the parameter page (Page increases by 1, loop to get the interface, total is 0, and the loop ends)<br/>2、Subsequent incremental update，obtain change data according to the parameter time increment (recommended request frequency：1min/time)
...
- **`results`** (`array of objects`) - Player list
  ...
  - **`honors`** (`array of objects`) - Honor list
    - **`honor`** (`object`) - Honor data
      - **`id`** (`string`) - Honor id
    ...
    - **`country_id`** (`string`) - Country/Region id（if it exists, it will be returned）
    - **`month`** (`integer`) - Month（if it exists, it will be returned）
    - **`week`** (`integer`) - Week（if it exists, it will be returned）
