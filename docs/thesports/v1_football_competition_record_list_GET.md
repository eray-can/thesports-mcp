...
**Endpoint**: `GET /v1/football/competition/record/list`

**Plan / Category**: `ADVANCED DATA`

**Included in Your Plan**: `Yes ✅`

**Description**: Return full competition record data，and obtain new or changed data according to the time<br/><br/>1、Full update for the first time，full data is obtained according to the parameter page (Page increases by 1, loop to get the interface, total is 0, and the loop ends)<br/>2、Subsequent incremental update，obtain change data according to the parameter time increment (recommended request frequency：1min/time)
...
| type | query | True | integer | List type, 1-youngest goalscorer record, 2-oldest goalscorer record |
...
- **`results`** (`array of objects`) - Team Ability
  - **`id`** (`string`) - Team id
  - **`record`** (`array of objects`) - Competition Record data
    - **`player_id`** (`string`) - Player id
    - **`time`** (`integer`) - Record time
    - **`age`** (`string`) - yy/mm/dd for the goal
