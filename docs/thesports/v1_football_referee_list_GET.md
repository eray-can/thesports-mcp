...
**Endpoint**: `GET /v1/football/referee/list`

**Plan / Category**: `BASIC INFO`

**Included in Your Plan**: `Yes ✅`

**Description**: Return full referee data，and obtain new or changed data according to the time<br/><br/>1、Full update for the first time，full data is obtained according to the parameter page (Page increases by 1, loop to get the interface, total is 0, and the loop ends)<br/>2、Subsequent incremental update，obtain change data according to the parameter time increment (recommended request frequency：1min/time)
...
- **`results`** (`array of objects`) - Referee list
  - **`id`** (`string`) - Referee id
  - **`name`** (`string`) - Referee name
  - **`short_name`** (`string`) - Referee abbreviation
  - **`logo`** (`string`) - Referee logo
  - **`birthday`** (`integer`) - birthday
  - **`country_id`** (`string`) - Country/Region id
  - **`uid`** (`string`) - Referee id (the corresponding id after the duplicate referees are merged), if it exists, it will be returned
