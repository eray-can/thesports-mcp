...
**Endpoint**: `GET /v1/football/language/list`

**Plan / Category**: `DATABASE DATA`

**Included in Your Plan**: `Yes ✅`

**Description**: Return full multi-language data (category, country/region, competition, team, player, injury) ，and obtain new or changed data according to the time<br/><br/>1、Full update for the first time，full data is obtained according to the parameter page (Page increases by 1, loop to get the interface, total is 0, and the loop ends)<br/>2、Subsequent incremental update，obtain change data according to the parameter time increment (recommended request frequency：1min/time)
...
| type | query | True | integer | List type, 1-category, 2-country/region, 3-competition, 4-team, 5-player, 6-injury(Team injury) |
...
- **`results`** (`array of objects`) - Language list
  - **`id`** (`string`) - Corresponding data id
  - **`name_*`** (`string`) - Other language names, no non-existent (*-language identification)
